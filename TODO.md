...
## Open Tasks
# TODO:
- [ ] 2025.03.29.21.31 H Rename `data_folder` to `user_folder`?
- [ ] 2025.03.29.20.51 H Rename `audio` option to `sound_volume` or `sound_level`.  Introduce `sound_card`.
- [ ] 2025.03.29.21.17 H Consider to pass in a list of parent options to each option: `sound_card` depends on `platform`, etc.
- [ ] 2025.03.29.21.11 H Go through all options and create a hierarchy for them.  Also rename `orientation` to `rotation` or so.
- [ ] 2025.03.29.21.12 H Introduce symbolic names for the four screen rotations: `none`, `left`, `right`, and `flip` (upside-down).
- [ ] 2025.03.29.21.02 H Add proper checks of all settings, especially `sound_card` and stuff that depends on other options.
- [ ] 2025.03.29.21.00 H Rewrite `Setup.save()` to use a new `Field` method, which returns the *external* representation (.INI).
- [ ] 2025.03.29.20.58 H Errors during editing are first caught by the main menu, so the user drops out to it on errors...
- [ ] 2025.03.29.20.53 H It still isn't possible to exit `KioskForge.py` if there are errors in the configuration (6 = force exit?).
- [ ] 2025.03.27.18.25 H Check if the HDMI1-audio-device `alsa_output.platform-fef00700.hdmi.hdmi-stereo` is system dependent:
                         It is currently hardcoded in the `KioskLaunchX11.sh` script generated by `KioskSetup.py`.
- [ ] 2025.03.27.18.57 H The output device should be user-selectable: HDMI1, HDMI2 or Jack. (on my Raspberry Pi 4B):
