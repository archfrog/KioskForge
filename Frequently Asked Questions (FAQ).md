...
that both are *case sensitive* so that `Test wifi` is different from `Test WIFI`.  You need to spell both exactly and precisely.

### I get weird errors about "set chanspec 0xNNNN fail, reason -52"
To be honest, I get a bunch of this error pretty much every time I install Ubuntu Server on the Raspberry Pi 4+.  I know that they
are related to the WIFI network card by Broadcomm, but I have no idea what they mean and why they pop up.
...
files to be removed again, so you should no longer run into this issue.  If you still run into this issue, please feel free to
report it.
