...
It does not matter the least if the script is ugly, a bit clumsy, or somewhat suboptimal in terms of execution speed or algorithms.
...
| ------------------------- | ---------------------------------------------------------------------------------------------- |
| `bcrypt`                  | A password hashing algorithm used to hash the user's password before forging the kiosk.        |
| `MyPy`                    | A static source code analyzer, which can detect a lot of potential issues in the code.         |
| `PyInstaller`             | Generates the single-file `.exe` for running KioskForge on Windows without installing Python.  |
