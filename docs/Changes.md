...
10. The `kiosk-disable-wifi-power-saving.sh` is no longer generated by the forge process and has been removed.  `KioskStart.py` now
    handles the task of disabling Wi-Fi power saving if the user has requested this by setting `wifi_boost` to `1`.  This fixes the
    issue that Wi-Fi power saving was only disabled during boot that did the actual forge process, not during latter boots.
