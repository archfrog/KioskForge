...
10. The `kiosk-disable-wifi-power-saving.sh` is no longer generated by the forge process and has been removed.  The new script
...
11. The `KioskSetup.py` script now takes an optional one-based step number, rather than the previous zero-based step number.
