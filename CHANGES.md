...
3. The cleaning of logs on the target kiosk machine now actually works (a lowercase 'm' was changed to an uppercase ditto).
4. The `cpu_boost` option is now honored properly.  Previously, all kiosks were created as if `cpu_boost` was always disabled.
5. KioskForge `.kiosk` files are now checked and validated *much* more thoroughly than before.
6. KioskForge now reports all detected issues and exits gracefully if you double-click on an invalid `.kiosk` file.
8. The HTML documentation in `C:\Program Files\KioskForge` is now much prettier than before.
7. KioskForge now runs using codepage 65001 (UTF-8) on Windows.
