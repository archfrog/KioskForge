...
1. Website URL (`website`): This should begin with `https://` and be the full address of the website you want to display.
2. Audio volume (`audio`): This should be non-zero only if the website in question has videos or other sources of audio.  It
...
3. Mouse cursor (`mouse`): This should be disabled (`0`) for a kiosk with a touch panel, to avoid showing a mouse cursor.
4. Idle timeout (`idle_timeout`): This should be set to a non-zero number of seconds between restarts of the browser so that
...
5. Data folder (`data_folder`): This value should be left blank as it is only useful for displaying a local file-based website.
