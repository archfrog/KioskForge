...
1. Kiosk type (`type`): This should be `web` to launch Chromium in X11.
2. Device type (`device`): This should be `pi4` for a plain Raspberry Pi 4, `pi4b` for a 4B model, and `pc` for a standard PC.
3. Website URL (`website`): This should begin with `https://` and be the full address of the website you want to display.
4. Audio volume (`audio`): This should be non-zero only if the website in question has videos or other sources of audio.  It
...
5. Mouse cursor (`mouse`): This should be disabled (`0`) for a kiosk with a touch panel, to avoid showing a mouse cursor.
6. Idle timeout (`idle_timeout`): This should be set to a non-zero number of seconds between restarts of the browser so that
...
7. Data folder (`data_folder`): This value should be left blank as it is only useful for displaying a local file-based website.
